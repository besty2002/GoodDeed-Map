...
export default function StoreDetail() {
  ...
  const shareToSNS = (platform: 'twitter' | 'line' | 'copy') => {
    ...
    const text = `【善행 맵】素敵な店主さんのいる「${store.name}」を紹介します。あなたの街にも、きっとあたたかい場所があります。 #善行マップ #温かい世界`;
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-16 pb-20 px-4 md:px-0">
      ...
      <section className="space-y-8">
        <div className="flex items-end gap-4"><h2 className="text-3xl font-black text-gray-900 tracking-tighter">出典・関連リンク</h2><span className="bg-gray-100 text-gray-400 text-xs px-3 py-1 rounded-full font-black uppercase tracking-widest">{store.sources?.length || 0} items</span></div>
        {store.sources && store.sources.length > 0 ? (
          <div className="grid gap-4">{store.sources.map((source) => (
            <a key={source.id} href={source.url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-6 md:p-8 bg-white border border-gray-100 rounded-[2.5rem] hover:shadow-2xl hover:shadow-orange-100/50 hover:-translate-x-1 transition-all group"><div className="flex items-center gap-6"><div className="p-4 bg-gray-50 rounded-2xl group-hover:bg-orange-50 transition-colors">{getSourceIcon(source.type)}</div><div className="space-y-1"><span className="text-[10px] font-black text-orange-600 uppercase tracking-widest">{source.type}</span><h3 className="font-black text-lg text-gray-800 group-hover:text-gray-900 transition-colors leading-tight">{source.title}</h3></div></div><div className="bg-gray-50 p-3 rounded-full group-hover:bg-orange-500 group-hover:text-white transition-all"><ExternalLink className="w-5 h-5" /></div></a>
          ))}</div>
