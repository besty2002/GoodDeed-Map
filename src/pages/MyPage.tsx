...
import { Heart, MessageCircle, Send, Loader2, MapPin, Gift, ArrowRight, User as UserIcon, Award, Star, Zap } from 'lucide-react';
      ...
      <header className="bg-white p-8 md:p-12 rounded-[4rem] border border-gray-100 shadow-2xl shadow-gray-200/40 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row items-center gap-10 relative z-10">
          <div className="relative">
            ...
            <div className={`absolute -bottom-4 -right-4 p-4 rounded-2xl bg-white shadow-xl border border-gray-50 text-transparent bg-clip-text bg-gradient-to-r ${userStats.currentBadge.color}`}>
          ...
          <div className="flex-1 text-center md:text-left space-y-4">
            <div className="space-y-1">
              <span className={`inline-block px-4 py-1 rounded-full text-[10px] font-black text-white bg-gradient-to-r ${userStats.currentBadge.color} tracking-[0.2em] mb-2`}>
                {userStats.currentBadge.name}
              </span>
            ...
            <div className="space-y-3 max-w-md">
              <div className="flex justify-between items-end">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Kindness Level Progress</span>
                <span className="text-sm font-black text-gray-900">{userStats.score} pts</span>
              </div>
              <div className="h-4 bg-gray-100 rounded-full overflow-hidden p-1 shadow-inner">
                <div 
                  className={`h-full rounded-full bg-gradient-to-r ${userStats.currentBadge.color} transition-all duration-1000`}
                  style={{ width: `${userStats.progress}%` }}
                />
              </div>
              {userStats.nextBadge && (
                <p className="text-[10px] font-bold text-gray-400 text-center md:text-left">
                  다음 등급 <span className="text-gray-600">[{userStats.nextBadge.name}]</span>까지 {userStats.nextBadge.minScore - userStats.score}포인트 남았습니다!
                </p>
              )}
            </div>
          </div>
...
function StatBadge({ icon, label, value, color }: { icon: JSX.Element, label: string, value: number, color: string }) {
  return (
